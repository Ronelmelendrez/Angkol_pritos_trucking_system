...
export function PayPeriodPicker({
  ...
  return (
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
      <Select
        value={frequency}
        onValueChange={(v) => onFrequencyChange(v as PayFrequencyFilter)}
      >
      ...
      <div className="flex items-center justify-between gap-1 sm:justify-start">
        ...
        <span className="min-w-0 flex-1 px-1 text-center text-sm font-medium text-ink sm:min-w-40 sm:flex-none">
          {isAll ? "All frequencies" : periodLabel}
        </span>
        <Button
          variant="outline"
          size="icon"
          onClick={onNext}
          disabled={isAll || !canGoNext}
        >
