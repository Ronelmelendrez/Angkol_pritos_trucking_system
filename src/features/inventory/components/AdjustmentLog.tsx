...
export function AdjustmentLog({ incidents }: Props) {
  ...
  const totalPages = useMemo(() => Math.max(1, Math.ceil(incidents.length / PAGE_SIZE)), [incidents.length]);
  const safePage = Math.min(page, totalPages);
  const pageItems = incidents.slice((safePage - 1) * PAGE_SIZE, safePage * PAGE_SIZE);
